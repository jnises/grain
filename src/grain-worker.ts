...
import { assertObject, assertImageData, assert } from './utils';
...
function safePostMessage(
  message: ProgressMessage | ResultMessage | ErrorMessage
): void {
...
self.onmessage = async function (e: MessageEvent<ProcessMessage>) {
  try {
    ...
    assert(type === 'process', 'Invalid message type received by worker', {
      receivedType: type,
