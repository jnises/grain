...
export class PerformanceTracker {
  ...
  endBenchmark(operation: string): PerformanceBenchmark | null {
    ...
    if (benchmark.pixelsProcessed) {
      benchmark.pixelsPerSecond =
        benchmark.pixelsProcessed / (benchmark.duration / 1000);
    }
  ...
  logSummary(): void {
    ...
    for (const benchmark of this.getAllBenchmarks()) {
      console.log(
        `${benchmark.operation}: ${benchmark.duration?.toFixed(2)}ms`
      );
      if (benchmark.pixelsPerSecond) {
        console.log(
          `  - ${(benchmark.pixelsPerSecond / 1000000).toFixed(2)}M pixels/sec`
        );
      }
    }
  }
  ...
  logIterationSummary(): void {
    ...
    const iterationBenchmarks = allBenchmarks.filter((b) =>
      b.operation.includes('Iteration')
    );
    const totalDevelopment = allBenchmarks.find(
      (b) => b.operation === 'Iterative Development'
    );
    ...
    if (totalDevelopment?.duration) {
      console.log(
        `Total Development Time: ${totalDevelopment.duration.toFixed(2)}ms`
      );
    }
    ...
    for (const [iterNum, benchmarks] of Array.from(
      iterationGroups.entries()
    ).sort(([a], [b]) => a - b)) {
      const iterationTotal = benchmarks.find(
        (b) => b.operation === `Development Iteration ${iterNum}`
      );
      console.log(`\nIteration ${iterNum}:`);
      ...
      for (const benchmark of benchmarks) {
        ...
        console.log(
          `  ${benchmark.operation.replace(`Iteration ${iterNum} - `, '')}: ${benchmark.duration?.toFixed(2)}ms`
        );
        if (benchmark.pixelsPerSecond) {
          console.log(
            `    - ${(benchmark.pixelsPerSecond / 1000000).toFixed(2)}M pixels/sec`
