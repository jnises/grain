...
export class KernelGenerator {
  ...
  generateSamplingKernel(grainRadius: number): SamplingKernel {
    devAssert(
      grainRadius > 0 && Number.isFinite(grainRadius),
      'grainRadius must be a positive finite number',
      { grainRadius }
    );
...
export function sampleGrainAreaExposure(
  ...
  devAssert(imageData.length > 0, 'imageData must not be empty', {
    length: imageData.length,
  });
  devAssert(
    imageData.length % 4 === 0,
    'imageData length must be divisible by 4 (RGBA format)',
    { length: imageData.length }
  );
  devAssert(Number.isFinite(grainX), 'grainX must be a finite number', {
    grainX,
  });
  devAssert(Number.isFinite(grainY), 'grainY must be a finite number', {
    grainY,
  });
  devAssert(
    grainRadius > 0 && Number.isFinite(grainRadius),
  ...
  devAssert(
    typeof kernelGenerator.generateSamplingKernel === 'function',
    'kernelGenerator must have generateSamplingKernel method',
    { kernelGenerator }
  );
