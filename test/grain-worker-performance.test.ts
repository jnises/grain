...
const mockPostMessage = (message: any) => {
  ...
  console.log(
    'Worker message:',
    message.type,
    message.stage || message.progress
  );
};
...
describe('Grain Generation Performance Integration Benchmarks', () => {
  it('runs variable grain size processing benchmark', async () => {
    ...
    console.log(`📊 Results:`);
    console.log(
      `  Low ISO (200): ${lowISOTime.toFixed(2)}ms (${lowISOResult.length} grains)`
    );
    console.log(
      `  High ISO (1600): ${highISOTime.toFixed(2)}ms (${highISOResult.length} grains)`
    );
    console.log(
      `  Grain count ratio: ${grainCountRatio.toFixed(2)}x more grains at high ISO`
    );
    console.log(`  Performance ratio: ${performanceRatio.toFixed(2)}x`);
    console.log(
      `  Low ISO throughput: ${(lowISOResult.length / (lowISOTime / 1000)).toFixed(0)} grains/sec`
    );
    console.log(
      `  High ISO throughput: ${(highISOResult.length / (highISOTime / 1000)).toFixed(0)} grains/sec`
    );
  }, 10000);
  ...
  it('runs image size scaling benchmark', async () => {
    ...
    for (const size of testSizes) {
      ...
      console.log(
        `    ${size.name}: ${time.toFixed(2)}ms (${grains.length} grains)`
      );
      console.log(
        `    Grains per second: ${(grains.length / (time / 1000)).toFixed(0)}`
      );
      console.log(
        `    Pixels per second: ${(pixelCount / (time / 1000) / 1000).toFixed(0)}K`
      );
    }
    ...
    if (results.length >= 2) {
      ...
      for (let i = 1; i < results.length; i++) {
        ...
        console.log(
          `    Efficiency: ${(grainRatio / timeRatio).toFixed(2)}x more grains per unit time`
        );
      }
    }
  }, 20000);
  ...
  it('runs variable grain size generation analysis benchmark', async () => {
    ...
    const sizeCounts = sizeRanges.map((range) => ({
      ...range,
      count: sizes.filter((size) => size >= range.min && size < range.max)
        .length,
    }));
    ...
    console.log(
      `  Pixel processing rate: ${(pixelsPerMs / 1000).toFixed(1)}K pixels/ms`
    );

    console.log(`\n📏 Size Distribution:`);
    console.log(
      `  Size range: ${minSize.toFixed(2)} - ${maxSize.toFixed(2)} (avg: ${avgSize.toFixed(2)})`
    );
    ...
    sizeCounts.forEach((range) => {
      ...
      console.log(
        `  ${range.name.padEnd(6)}: ${range.count.toString().padStart(4)} grains (${percentage}%)`
      );
    });
    ...
    for (let i = 0; i < Math.min(5, grains.length); i++) {
      ...
      console.log(
        `  Grain ${i + 1}: pos(${grain.x.toFixed(1)}, ${grain.y.toFixed(1)}) size=${grain.size.toFixed(2)} sensitivity=${grain.sensitivity.toFixed(2)}`
      );
    }
  }, 15000);
});
