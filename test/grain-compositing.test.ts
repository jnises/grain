...
describe('Variable Grain Size Generation', () => {
  ...
  test('should generate grains with variable sizes', { timeout: 10000 }, () => {
    ...
    for (const grain of grains) {
      ...
      assert(
        typeof grain.sensitivity === 'number' && grain.sensitivity > 0,
        'Grain sensitivity must be positive',
        { grain }
      );
    }
  ...
  test(
    'should have size variation within expected range',
    { timeout: 10000 },
    () => {
      const grains = generator.generateGrainStructure();
      ...
      assert(
        Array.isArray(grains) && grains.length > 0,
        'Need grains to analyze',
        { grains }
      );
  ...
  test(
    'should generate grains with proper distribution',
    { timeout: 10000 },
    () => {
      const grains = generator.generateGrainStructure();
...
describe('Grain Structure Properties', () => {
  test('should generate consistent grain properties', () => {
    ...
    if (grains.length > 0) {
      ...
      assert(typeof firstGrain.x === 'number', 'Grain x must be a number', {
        firstGrain,
      });
      assert(typeof firstGrain.y === 'number', 'Grain y must be a number', {
        firstGrain,
      });
      assert(
        typeof firstGrain.size === 'number',
        'Grain size must be a number',
        { firstGrain }
      );
      assert(
        typeof firstGrain.sensitivity === 'number',
