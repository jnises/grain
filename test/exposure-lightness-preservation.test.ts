...
describe('Exposure Lightness Preservation', () => {
  ...
  it.skip('should preserve overall lightness for middle gray (18% gray)', async () => {
    ...
    const grainProcessor = createTestGrainProcessor(
      width,
      height,
      DEFAULT_SETTINGS
    );
    ...
    console.log(
      `Lightness change: ${(((outputLightness - inputLightness) / inputLightness) * 100).toFixed(2)}%`
    );

    // Allow larger tolerance in linear space due to non-linear relationship with perceived lightness
    const tolerance = 0.2; // 20% tolerance in linear space
    const lightnessDifference =
      Math.abs(outputLightness - inputLightness) / inputLightness;
  ...
  it.skip('should preserve overall lightness for various gray levels', async () => {
    ...
    for (const grayValue of testGrayValues) {
      ...
      console.log(
        `Gray ${grayValue}: Input ${inputLightness.toFixed(2)} -> Output ${outputLightness.toFixed(2)} (${(((outputLightness - inputLightness) / inputLightness) * 100).toFixed(2)}% change)`
      );
  ...
  it.skip('should preserve overall lightness for black and white extremes', async () => {
    ...
    const grainProcessor1 = createTestGrainProcessor(
      width,
      height,
      DEFAULT_SETTINGS
    );
    const blackResult = await grainProcessor1.processImage(
      blackImage as ImageData
    );
    ...
    console.log(
      `Black: Input ${blackInputLightness.toFixed(2)} -> Output ${blackOutputLightness.toFixed(2)}`
    );
    ...
    const grainProcessor2 = createTestGrainProcessor(
      width,
      height,
      DEFAULT_SETTINGS
    );
    const whiteResult = await grainProcessor2.processImage(
      whiteImage as ImageData
    );
    ...
    console.log(
      `White: Input ${whiteInputLightness.toFixed(2)} -> Output ${whiteOutputLightness.toFixed(2)} (${(((whiteOutputLightness - whiteInputLightness) / whiteInputLightness) * 100).toFixed(2)}% change)`
    );

    // White should not be significantly darkened by grain (larger tolerance in linear space)
    const whiteLightnessDifference =
      Math.abs(whiteOutputLightness - whiteInputLightness) /
      whiteInputLightness;
