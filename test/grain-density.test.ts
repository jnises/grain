...
describe('GrainDensityCalculator', () => {
  ...
  describe('calculateIntrinsicGrainDensities', () => {
    it('should produce uniform densities for uniform grains with uniform exposures', () => {
      ...
      const densityMap = calculator.calculateIntrinsicGrainDensities(uniformGrains, exposureMap);
      ...
      const mean = densityValues.reduce((sum, val) => sum + val, 0) / densityValues.length;
      const variance = densityValues.reduce((sum, val) => sum + (val - mean) ** 2, 0) / densityValues.length;
      ...
      console.log(`Debug - Density Mean: ${mean.toFixed(4)}, StdDev: ${stdDev.toFixed(4)}, Min: ${minDensity.toFixed(4)}, Max: ${maxDensity.toFixed(4)}`);
    ...
    it('should handle empty grain array', () => {
      ...
      const densityMap = calculator.calculateIntrinsicGrainDensities(emptyGrains, emptyExposureMap);
    ...
    it('should produce different densities for different exposures', () => {
      ...
      const grains: GrainPoint[] = [
        { x: 10, y: 10, size: 2.0, sensitivity: 0.5, developmentThreshold: 0.1 },
        { x: 20, y: 20, size: 2.0, sensitivity: 0.5, developmentThreshold: 0.1 },
      ];
      ...
      const densityMap = calculator.calculateIntrinsicGrainDensities(grains, exposureMap);
