...
describe('Development Threshold System', () => {
  ...
  describe('Per-Grain Development Thresholds', () => {
    ...
    it('should calculate thresholds based on film type characteristics', () => {
      ...
      const kodakAvg =
        kodakGrains.reduce((sum, g) => sum + g.developmentThreshold, 0) /
        kodakGrains.length;
      const fujiAvg =
        fujiGrains.reduce((sum, g) => sum + g.developmentThreshold, 0) /
        fujiGrains.length;
      const ilfordAvg =
      ...
      const fujiBase =
        FILM_CHARACTERISTICS.fuji.developmentThreshold.baseSensitivity;
      const ilfordBase =
    ...
    it('should vary thresholds based on grain size', () => {
      ...
      const smallAvgThreshold =
        smallGrains.reduce((sum, g) => sum + g.developmentThreshold, 0) /
        smallGrains.length;
      const largeAvgThreshold =
        largeGrains.reduce((sum, g) => sum + g.developmentThreshold, 0) /
        largeGrains.length;
  ...
  describe('Grain Activation Logic', () => {
    it('should not activate grains below development threshold', () => {
      ...
      const strength = testCalculator.testCalculateIntrinsicGrainDensity(
        lowExposure,
        testGrain
      );
    ...
    it('should show sigmoid activation behavior', () => {
      ...
      console.log(
        'Exposure vs Strength:',
        exposures.map((exp, i) => `${exp}: ${strengths[i].toFixed(4)}`)
      );
  ...
  describe('Deterministic Behavior', () => {
    it('should produce consistent results for same grain and exposure', () => {
      ...
      const exposure = 0.7;
      const strength1 = testCalculator.testCalculateIntrinsicGrainDensity(
        exposure,
        testGrain
      );
      const strength2 = testCalculator.testCalculateIntrinsicGrainDensity(
        exposure,
        testGrain
      );
  ...
  describe('Integration with Film Characteristics', () => {
    it('should respect film-specific development parameters', () => {
      ...
      const testIlfordCalculator = new TestGrainDensityCalculator(
        ilfordSettings
      );
      ...
      const exposure = 1.0;
      const kodakStrength =
        testKodakCalculator.testCalculateIntrinsicGrainDensity(
          exposure,
          kodakGrain
        );
      const ilfordStrength =
        testIlfordCalculator.testCalculateIntrinsicGrainDensity(
          exposure,
          ilfordGrain
        );

      console.log(
        `Kodak: threshold=${kodakGrain.developmentThreshold}, strength=${kodakStrength.toFixed(4)}`
      );
      console.log(
        `Ilford: threshold=${ilfordGrain.developmentThreshold}, strength=${ilfordStrength.toFixed(4)}`
      );
