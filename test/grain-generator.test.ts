...
describe('GrainGenerator', () => {
  ...
  describe('Method Input Validation', () => {
    it('should validate generatePoissonDiskSampling parameters', () => {
      expect(() => generator.generatePoissonDiskSampling(0, 100)).toThrow(
        /mindistance.*positive.*finite.*number/i
      );
      expect(() => generator.generatePoissonDiskSampling(-5, 100)).toThrow(
        /mindistance.*positive.*finite.*number/i
      );
      expect(() => generator.generatePoissonDiskSampling(10, 0)).toThrow(
        /maxsamples.*positive.*integer/i
      );
      expect(() => generator.generatePoissonDiskSampling(10, -5)).toThrow(
        /maxsamples.*positive.*integer/i
      );
      expect(() => generator.generatePoissonDiskSampling(10, 1.5)).toThrow(
        /maxsamples.*positive.*integer/i
      );
    });
    ...
    it('should validate generateFallbackGrains parameters', () => {
      ...
      expect(() =>
        generator.generateFallbackGrains('invalid' as any, 100)
      ).toThrow(/existinggrains.*array/i);
      expect(() => generator.generateFallbackGrains([], 0)).not.toThrow(); // 0 is valid (returns empty array)
      expect(() => generator.generateFallbackGrains([], -5)).toThrow(
        /targetcount.*non-negative.*finite.*number/i
      );
      expect(() => generator.generateFallbackGrains([], 1.5)).toThrow(
        /targetcount.*integer/i
      );
    });
    ...
    it('should validate analyzeDistribution parameters', () => {
      ...
      expect(() => generator.analyzeDistribution('invalid' as any)).toThrow(
        /grains.*array/
      );
    ...
    it('should validate createGrainGrid parameters', () => {
      const validGrains = [
        { x: 10, y: 10, size: 2, sensitivity: 1.0, developmentThreshold: 0.5 },
      ];
      expect(() => generator.createGrainGrid(null as any)).toThrow(
        /grains.*array/
      );
      expect(() => generator.createGrainGrid('invalid' as any)).toThrow(
        /grains.*array/
      );
  ...
  describe('Method Precondition Validation', () => {
    it('should validate grain objects in arrays', () => {
      ...
      for (const invalidGrain of invalidGrains) {
        expect(() =>
          generator.createGrainGrid([invalidGrain as any])
        ).toThrow();
      }
    });
  ...
  describe('Parameter Calculations', () => {
    it('should calculate grain parameters correctly for ISO 400', () => {
      ...
      assert(
        typeof params.baseGrainSize === 'number' && params.baseGrainSize > 0,
        'Base grain size must be a positive number',
        { params }
      );
      assert(
        typeof params.imageArea === 'number' && params.imageArea > 0,
        'Image area must be a positive number',
        { params }
      );
      assert(
        typeof params.densityFactor === 'number' && params.densityFactor >= 0,
        'Density factor must be a non-negative number',
        { params }
      );
      assert(
        typeof params.grainDensity === 'number' && params.grainDensity >= 0,
        'Grain density must be a non-negative number',
        { params }
      );
      assert(
        typeof params.minDistance === 'number' && params.minDistance > 0,
        'Min distance must be a positive number',
        { params }
      );
    ...
    it('should handle low ISO values correctly', () => {
      ...
      assert(
        params.baseGrainSize >= 0.5,
        'Base grain size should have minimum value',
        { params }
      );
      assert(
        params.densityFactor > 0,
        'Density factor should be positive for valid ISO',
        { params }
      );
      assert(params.grainDensity > 0, 'Grain density should be positive', {
        params,
      });
    ...
    it('should handle high ISO values correctly', () => {
      ...
      assert(params.baseGrainSize > 0, 'Base grain size should be positive', {
        params,
      });
      assert(
        params.densityFactor <= 0.8,
        'Density factor should be capped at 0.8',
        { params }
      ); // Updated max density factor
      assert(params.grainDensity > 0, 'Grain density should be positive', {
        params,
      });
    ...
    it('should scale with image size', () => {
      ...
      assert(
        params.imageArea === 800 * 600,
        'Image area should match dimensions',
        { params }
      );
      assert(
        params.grainDensity > generator.calculateGrainParameters().grainDensity,
  ...
  describe('Poisson Disk Sampling', () => {
    it('should generate points within image bounds', () => {
      ...
      for (const point of points) {
        ...
        assert(typeof point.x === 'number', 'Point x must be a number', {
          point,
        });
        assert(typeof point.y === 'number', 'Point y must be a number', {
          point,
        });
    ...
    it('should respect minimum distance constraint for most points', () => {
      ...
      assert(Array.isArray(points), 'Points must be an array', { points });
      assert(points.length > 0, 'Should generate at least one point', {
        points,
      });
    ...
    it('should generate fewer points with larger minimum distance', () => {
      const pointsSmallDistance = generator.generatePoissonDiskSampling(
        5,
        1000
      );
      const pointsLargeDistance = generator.generatePoissonDiskSampling(
        50,
        1000
      );

      expect(pointsLargeDistance.length).toBeLessThan(
        pointsSmallDistance.length
      );
    });
  });
  ...
  describe('Distribution Analysis', () => {
    ...
    it('should calculate distances between grains', () => {
      ...
      if (grains.length > 1) {
        ...
        expect(analysis.minDistance!).toBeLessThanOrEqual(
          analysis.medianDistance!
        );
        expect(analysis.medianDistance!).toBeLessThanOrEqual(
    ...
    it('should have balanced distribution across quadrants', () => {
      ...
      expect(analysis.quadrants.topLeft).toBeGreaterThan(
        expectedPerQuadrant - tolerance
      );
      expect(analysis.quadrants.topLeft).toBeLessThan(
        expectedPerQuadrant + tolerance
      );
      expect(analysis.quadrants.topRight).toBeGreaterThan(
        expectedPerQuadrant - tolerance
      );
      expect(analysis.quadrants.topRight).toBeLessThan(
        expectedPerQuadrant + tolerance
      );
      expect(analysis.quadrants.bottomLeft).toBeGreaterThan(
        expectedPerQuadrant - tolerance
      );
      expect(analysis.quadrants.bottomLeft).toBeLessThan(
        expectedPerQuadrant + tolerance
      );
      expect(analysis.quadrants.bottomRight).toBeGreaterThan(
        expectedPerQuadrant - tolerance
      );
      expect(analysis.quadrants.bottomRight).toBeLessThan(
        expectedPerQuadrant + tolerance
      );
    });
  });
  ...
  describe('Spatial Grid Creation', () => {
    ...
    it('should place grains in appropriate grid cells', () => {
      ...
      const found =
        nearbyGrains.includes(testGrain) ||
  ...
  describe('Random Number Generation', () => {
    ...
    it('should produce different results with different seeds', () => {
      ...
      if (points1.length !== points2.length) {
        ...
        for (let i = 0; i < points1.length; i++) {
          if (
            Math.abs(points1[i].x - points2[i].x) > 0.001 ||
