...
import { calculateGrainFalloff, calculateSampleWeight } from '../src/grain-math';
import type { GrainPoint } from '../src/types';

// Test utility function - create grayscale image data
function createGrayscaleImageData(width: number, height: number, luminancePattern: (x: number, y: number) => number): { data: Float32Array } {
...
describe('Grain Weighting Consistency', () => {
  ...
  describe('Single grain behavior consistency', () => {
    it('should produce consistent behavior between exposure calculation and pixel effects', async () => {
      ...
      const imageData = createGrayscaleImageData(width, height, () => uniformGray);
      ...
      const intrinsicDensity = grainDensityCalculator['calculateIntrinsicGrainDensity'](grainExposure, grain);
    ...
    it('should scale correctly with different grain sizes', async () => {
      ...
      grainSizes.forEach(grainSize => {
        ...
        if (grainSize > 3.0) {
          ...
          const smallIntrinsicDensity = grainDensityCalculator['calculateIntrinsicGrainDensity'](grainExposure, smallGrain);
          const smallPixelEffect = grainDensityCalculator.calculatePixelGrainEffect(
    ...
    it('should produce mathematically consistent grain shapes', () => {
      ...
      const falloffValues = testDistances.map(d => calculateGrainFalloff(d, grainRadius));
