...
describe('Iterative vs Single-Pass Approach Quality', () => {
   ...
   */
  function calculateAverageLightness(imageData: {
   ...
   */
  function calculateLightnessError(
  ...
  it('should demonstrate improved lightness preservation with iterative approach', async () => {
    ...
    const singlePassProcessor = createTestGrainProcessor(
      width,
      height,
      singlePassSettings
    );
    ...
    const iterativeProcessor = createTestGrainProcessor(
      width,
      height,
      iterativeSettings,
      12346
    ...
    const singlePassError = calculateLightnessError(
      inputImage,
      singlePassResult
    );
    ...
    console.log(
      `Single-pass result: ${singlePassLightness.toFixed(4)} (error: ${(singlePassError * 100).toFixed(2)}%)`
    );
    console.log(
      `Iterative result: ${iterativeLightness.toFixed(4)} (error: ${(iterativeError * 100).toFixed(2)}%)`
    );
    console.log(
      `Improvement factor: ${(singlePassError / iterativeError).toFixed(2)}x better`
    );
    ...
    expect(singlePassError).toBeLessThan(0.1); // Within 10% error for single-pass

    // When both approaches perform extremely well (under 0.02% error), the improvement may be negligible
    // This is expected after the darkroom physics fixes that made both approaches highly accurate
    const bothExcellent = iterativeError < 0.0002 && singlePassError < 0.0002;

    if (bothExcellent) {
      // For extremely accurate results, allow small tolerance for measurement variance
      expect(iterativeError).toBeLessThanOrEqual(singlePassError + 0.0001);
      console.log(
        'Both approaches achieved excellent accuracy - difference within measurement tolerance'
      );
    } else {
      // For cases where there's room for improvement, iterative should be better
      expect(iterativeError).toBeLessThan(singlePassError);
      const improvementFactor = singlePassError / iterativeError;
      expect(improvementFactor).toBeGreaterThan(1.0);
    }
  });
  ...
  it('should demonstrate quality improvements across different gray levels', async () => {
    ...
    for (const grayValue of testGrayValues) {
      ...
      const singlePassResult =
        await singlePassProcessor.processImage(inputImage);
      ...
      const iterativeError = calculateLightnessError(
        inputImage,
        iterativeResult
      );
      ...
      console.log(
        `Gray ${grayPercent}%: Single-pass error ${(singlePassError * 100).toFixed(2)}%, Iterative error ${(iterativeError * 100).toFixed(2)}%`
      );
  ...
  it('should show convergence behavior with different iteration limits', async () => {
    ...
    console.log(
      `Input lightness: ${calculateAverageLightness(inputImage).toFixed(4)}`
    );
    ...
    for (const maxIterations of iterations) {
      ...
      console.log(
        `${maxIterations} iteration(s): ${(error * 100).toFixed(2)}% error`
      );
    }
