...
describe('calculateLightnessFactor', () => {
  ...
  it('should throw error for empty arrays', () => {
    ...
    expect(() => calculateLightnessFactor(empty, nonEmpty)).toThrow(
      'originalData must not be empty'
    );
  });
  ...
  it('should throw error for mismatched array lengths', () => {
    ...
    expect(() => calculateLightnessFactor(short, long)).toThrow(
  ...
  it('should throw error for non-RGBA format (length not divisible by 4)', () => {
    ...
    expect(() =>
      calculateLightnessFactor(badLength, matchingBadLength)
    ).toThrow('data length must be divisible by 4 (RGBA format)');
  });

  it('should preserve precision for edge cases', () => {
    // Test with very small differences
    const original = createRgbaArray([0.100001]);
    const processed = createRgbaArray([0.1]);
