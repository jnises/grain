...
async function testGrainExposureVariability(
  ...
  for (const grain of grains) {
    ...
    if (x >= 0 && x < width && y >= 0 && y < height) {
      ...
      const kernelExposure =
        validSamples > 0 ? totalExposure / validSamples : pointExposure;
  ...
  const stability =
    pointStats.variance > 0 ? kernelStats.variance / pointStats.variance : 1;
...
describe('Kernel-based Grain Area Sampling Quality Validation', () => {
  describe('Exposure Stability for Different Grain Sizes', () => {
    it.skip('should provide more stable exposure values for larger grains', async () => {
      ...
      const largeGrainResults = await testGrainExposureVariability(
        'gradient',
        5.0
      );

      // Larger grains should benefit more from kernel sampling (more stable)
      expect(largeGrainResults.stability).toBeLessThan(
        smallGrainResults.stability
      );
  ...
  describe('Boundary Condition Handling', () => {
    it('should handle grains near image edges gracefully', async () => {
      ...
      function simulateEdgeGrainSampling(
        x: number,
        y: number,
        grainSize: number
      ): boolean {
  ...
  describe('Sampling Quality and Performance', () => {
    ...
    it('should show greater benefit for larger grains', async () => {
      ...
      const smallGrain = await testGrainExposureVariability(
        'checkerboard',
        1.5
      );
      const mediumGrain = await testGrainExposureVariability(
        'checkerboard',
        3.0
      );
      const largeGrain = await testGrainExposureVariability(
        'checkerboard',
        6.0
      );
    ...
    it('should validate that kernel sampling smooths exposure variations', async () => {
      ...
      const highNoiseResults = await testGrainExposureVariability(
        'checkerboard',
        3.0
      );
...
describe('Kernel Sampling Integration', () => {
  it('should process grain with kernel-based sampling', async () => {
    ...
    const distributionAnalysis =
      grainGenerator.analyzeDistribution(basicGrains);
    ...
    expect(distributionAnalysis.quadrants.bottomRight).toBeGreaterThanOrEqual(
      0
    );

    if (
      distributionAnalysis.minDistance !== undefined &&
      distributionAnalysis.maxDistance !== undefined
    ) {
      expect(distributionAnalysis.maxDistance).toBeGreaterThanOrEqual(
        distributionAnalysis.minDistance
      );
    }
    ...
    console.log(
      `   - Settings: ISO ${settings.iso}, ${settings.filmType} film`
    );
    console.log(
      `   - Base grain size: ${grainParams.baseGrainSize.toFixed(2)}`
    );
    console.log(`   - Actual grain count: ${grains.length}`);
    console.log(
      `   - Coverage: ${(distributionAnalysis.coverage * 100).toFixed(3)}%`
    );
    console.log(
      `   - Density: ${distributionAnalysis.density.toFixed(1)} grains per 1000 pixels`
    );
  });
});
