...
describe('Grain Distribution Bug Tests', () => {
  ...
  describe('Distribution Coverage Tests', () => {
    it('should distribute grains across entire image area', () => {
      ...
      assert(grains.length > 0, 'Should generate at least some grains', {
        grains,
      });
      ...
      for (const grain of grains) {
        ...
        assert(grain.x >= 0 && grain.x < 400, 'Grain x must be within bounds', {
          grain,
        });
        assert(grain.y >= 0 && grain.y < 300, 'Grain y must be within bounds', {
          grain,
        });
      ...
      const regionsWithGrains = Object.values(regionCounts).filter(
        (count) => count > 0
      ).length;
    ...
    it('should not cluster grains in corner or edge areas', () => {
      ...
      for (const grain of grains) {
        ...
        assertObject(grain, 'Grain must be an object');
        assert(
          typeof grain.x === 'number' && typeof grain.y === 'number',
          'Grain coordinates must be numbers',
          { grain }
        );
        ...
        const isInCenter =
          grain.x > cornerSize * 2 &&
          grain.x < 400 - cornerSize * 2 &&
          grain.y > edgeSize * 2 &&
          grain.y < 300 - edgeSize * 2;
      ...
      console.log(
        `Corner grains: ${cornerGrains}, Center grains: ${centerGrains}, Total: ${grains.length}`
      );
    ...
    it(
      'should maintain consistent density across different image sizes',
      { timeout: 15000 },
      () => {
        const sizes = [
  ...
  describe('Poisson Distribution Coverage Tests', () => {
    ...
    it('should distribute Poisson samples across entire image area', () => {
      ...
      assert(Array.isArray(poissonPoints), 'Poisson points must be an array', {
        poissonPoints,
      });
      ...
      if (poissonPoints.length < 50) {
        console.log(
          `Skipping Poisson distribution test - only ${poissonPoints.length} points generated`
        );
        return;
      }
      ...
      for (const point of poissonPoints) {
        ...
        assert(
          ...
          'Poisson point x must be a number',
          { point }
        );
        assert(
          typeof point.y === 'number',
          'Poisson point y must be a number',
          { point }
        );
        assert(
          point.x >= 0 && point.x < 400,
          'Poisson point x must be within bounds',
          { point }
        );
        assert(
          point.y >= 0 && point.y < 300,
          'Poisson point y must be within bounds',
          { point }
        );
      ...
      const regionsWithPoints = Object.values(regionCounts).filter(
        (count) => count > 0
      ).length;
    ...
    it('should maintain Poisson minimum distance constraints', () => {
      ...
      if (poissonPoints.length < 2) {
        console.log(
          `Skipping distance constraint test - only ${poissonPoints.length} points generated`
        );
        return;
      }

      console.log(
        `Testing ${poissonPoints.length} Poisson points for min distance ${params.minDistance}`
      );
      ...
      console.log(
        `Distance violations: ${violationCount} out of ${(poissonPoints.length * (poissonPoints.length - 1)) / 2} pairs`
      );
    ...
    it('should not cluster Poisson samples in corner or edge areas', () => {
      ...
      if (poissonPoints.length < 20) {
        console.log(
          `Skipping Poisson clustering test - only ${poissonPoints.length} points generated`
        );
        return;
      }
      ...
      for (const point of poissonPoints) {
        ...
        assert(
          ...
          'Poisson point coordinates must be numbers',
          { point }
        );
        ...
        const isInCenter =
          point.x > cornerSize * 2 &&
          point.x < 400 - cornerSize * 2 &&
          point.y > edgeSize * 2 &&
          point.y < 300 - edgeSize * 2;
      ...
      console.log(
        `Poisson - Corner points: ${cornerPoints}, Center points: ${centerPoints}, Total: ${poissonPoints.length}`
      );
  ...
  describe('Poisson vs Fallback Behavior', () => {
    it('should trigger fallback when Poisson sampling is insufficient', () => {
      ...
      console.log(
        `Poisson generated: ${poissonPoints.length} / ${params.grainDensity} target`
      );
    ...
    it('should test fallback grid generation specifically', () => {
      ...
      console.log(
        `Fallback generated: ${fallbackGrains.length} for target ${targetCount}`
      );
  ...
  describe('Parameter Validation', () => {
    it('should have reasonable minimum distances', () => {
      ...
      for (const testCase of testCases) {
        ...
        console.log(
          `ISO ${testCase.iso}: minDistance=${params.minDistance}, grainSize=${params.baseGrainSize}`
        );
