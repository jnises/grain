...
describe('Lightness Iteration Convergence', () => {
  ...
  describe('calculateLightnessFactor and adjustGrainExposures integration', () => {
    ...
    it('should converge toward target lightness through successive iterations', () => {
      ...
      while (
        Math.abs(currentLightnessFactor - targetLightness) >
        ...
        currentLightnessFactor =
          targetLightness +
          (currentLightnessFactor - targetLightness) * dampening;
      ...
      console.log(
        `Factor history: ${factorHistory.map((f) => f.toFixed(3)).join(' → ')}`
      );
      ...
      expect(Math.abs(currentLightnessFactor - targetLightness)).toBeLessThan(
        convergenceThreshold
      );
      ...
      if (factorHistory.length > 1) {
        ...
        expect(Math.abs(lastFactor - targetLightness)).toBeLessThan(
          Math.abs(firstFactor - targetLightness)
        );
      }
    });
    ...
    it('should handle edge cases in convergence', () => {
      ...
      const adjusted = TestGrainProcessor.testAdjustGrainExposures(
        exposureMap,
        verySmallFactor
      );
      ...
      const adjusted2 = TestGrainProcessor.testAdjustGrainExposures(
        exposureMap,
        veryLargeFactor
      );
