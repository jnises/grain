...
async function runProfilingTest() {
  ...
  console.log(
    `Configuration: ${TEST_CONFIG.width}x${TEST_CONFIG.height}, ISO ${TEST_CONFIG.settings.iso}`
  );
  ...
  console.log(
    `✅ Completed ${iterations} iterations in ${totalTime.toFixed(2)}ms`
  );
  console.log(`Average time per iteration: ${avgTime.toFixed(2)}ms`);
  console.log('');
  console.log('🔥 To analyze the profiling results:');
  console.log(
    '1. Find the generated isolate-*.log file in the current directory'
  );
  console.log(
    '2. Process it with: node --prof-process isolate-*.log > profile.txt'
  );
  console.log(
    '3. Look for the "Bottom up (heavy) profile" section in profile.txt'
  );
  console.log(
    '4. Functions with high "self" percentages are the hotspots to optimize'
  );
}

async function main() {
  console.log(
    '🔍 Running grain processing performance test (PRODUCTION MODE)...'
  );
  ...
  if (!process.execArgv.some((arg) => arg.includes('--prof'))) {
    ...
    console.log(
      '  node --prof --loader tsx/esm scripts/profiling/simple_profiler.ts'
    );
