...
async function profileConfiguration(
  ...
  console.log(
    `🔍 Profiling ${config.name} (${config.width}x${config.height}, ISO ${config.settings.iso})...`
  );
  ...
  const outputPath = join(
    ...
    'profiling',
    'output',
    `profile-${config.name}-${Date.now()}.cpuprofile`
  );
  ...
  const profile = await new Promise<object>((resolve) => {
    session.post(
      'Profiler.stop',
      (err: Error | null, result: { profile: object }) => {
        if (err) throw err;
...
function analyzeProfile(profilePath: string): void {
  ...
  try {
    ...
    for (let i = 0; i < sortedFunctions.length; i++) {
      ...
      const funcNameShort =
        functionName.length > 45
          ? functionName.substring(0, 42) + '...'
          : functionName;

      console.log(
        `│ ${rank} │ ${selfTimeStr} │ ${totalTimeStr} │ ${hitCountStr} │ ${funcNameShort}`
      );
    }
    ...
    const totalSelfTime = Array.from(functionStats.values()).reduce(
      (sum, stats) => sum + stats.selfTime,
      0
    );
    ...
    if (sortedFunctions.length > 0) {
      ...
      console.log(
        `  └─ ${topFunction[1].selfTime.toFixed(2)}μs (${topPercentage.toFixed(1)}% of total CPU time)`
      );

      // Look for grain processing specific functions
      const grainFunctions = sortedFunctions.filter(
        ([name]) =>
          name.includes('grain') ||
      ...
      if (grainFunctions.length > 0) {
        ...
        for (const [functionName, stats] of grainFunctions.slice(0, 5)) {
          ...
          console.log(
            `  • ${functionName}: ${stats.selfTime.toFixed(2)}μs (${percentage.toFixed(1)}%)`
          );
        }
      }
    }
...
function generateRecommendations(profilePaths: string[]): void {
  ...
  console.log(
    '1. Focus optimization efforts on the functions with highest self time'
  );
  console.log(
    '2. Functions with high hit count but low self time may benefit from inlining'
  );
  console.log(
    '3. Look for opportunities to reduce computational complexity in top hotspots'
  );
  console.log(
    '4. Consider caching or memoization for frequently called functions'
  );
  ...
  console.log(
    '   • speedscope.app - Upload .cpuprofile files for flamegraph visualization'
  );
  console.log('   • clinic.js - Install with: npm install -g clinic');
  console.log(
    '   • --prof flag - Use Node.js built-in profiler for V8-level analysis'
  );
...
async function main(): Promise<void> {
  console.log(
    '🚀 Starting flamegraph profiling for grain processing hotspots...\n'
  );
